...
namespace SimpleProject.Application.EventHandlers
{
    public class OrderEventHandler
    {
        ...
        {
            await _orderService.Cancel(orderEvent.Account, orderEvent.DynamicRouteRequest, orderEvent.Order, orderEvent.Transaction);
        }
        ...
        public async Task HandleCancelled(OrderEvent orderEvent)
        {
            ...
            if (orderEvent.Transaction.State == TransactionState.Authorized)
            {
                ...
                {
                    Account = orderEvent.Account,
                    DynamicRouteRequest = orderEvent.DynamicRouteRequest,
        ...
        {
            await _orderService.Complete(orderEvent.Account, orderEvent.DynamicRouteRequest, orderEvent.Order, orderEvent.Transaction);
        }
        ...
        {
            await _orderService.Create(orderEvent.Account, orderEvent.DynamicRouteRequest, orderEvent.Order,  orderEvent.Transaction);
        }
        ...
        {
            await _orderService.Process(orderEvent.Account, orderEvent.DynamicRouteRequest, orderEvent.Order, orderEvent.Transaction);
        }
    }
}
